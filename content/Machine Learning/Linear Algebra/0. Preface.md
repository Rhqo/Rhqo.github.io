...
> 	Matrix $A$는 노드들 간의 연결(엣지, edges)을 나타낸다.
...
> [!NOTE] Three questions before course get serious
> #### 1. 세 선분의 길이로 삼각형을 만들 수 있는 조건
> 길이가 r, s, 그리고 t인 세 개의 직선(line segments)을 이 페이지에 그렸다고 하자. \
> 이 세 선분으로 삼각형을 만들기 위해 r, s, t의 길이에 필요한 조건은 무엇인가? \
> 이 질문에서는 세 직선의 방향을 자유롭게 선택할 수 있다. \
> #### 2. 고정된 방향을 가지는 벡터로 삼각형을 만들 수 있는지  
> 이제 세 개의 직선 방향이 각각 다른 u, v, w 벡터로 고정되어 있다고 가정하자. \
> 그러나 이 선들의 길이를 조절하여 au, bv, cw로 늘리거나 줄일 수 있다고 하자 (a, b, c는 스칼라이다).  
> 이 세 벡터 au, bv, cw로 항상 닫힌 삼각형(closed triangle)을 만들 수 있는가?
> #### 3. 3차원 공간에서 네 벡터로 합이 0인 식을 만들 수 있는지  
> Linear Algebra는 평면에만 머무르지 않는다! 
> 3차원 공간에서 서로 다른 방향을 가지는 네 개의 직선 u, v, w, z가 있다고 가정하자.  
> 이 경우 a,b,c,d (여기서 0은 허용되지 않음)를 선택하여
