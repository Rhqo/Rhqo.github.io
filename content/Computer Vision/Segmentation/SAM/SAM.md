...
이러한 능력은 “Prompt Engineering”이라는 것을 통해 구현되는데, prompt engineering은 직접 만든 텍스트로 언어 모델이 직면한 작업에 대한 유효한 텍스트 응답을 생성하도록 유도하는 데 사용된다.
충분한 데이터셋으로 scale되고 train된다면, 모델의 zero-shot과 few-shot의 성능은 그 fine tuned된 모델과 잘 비교된다.

Foundation model은 그 수가 많지는 않지만, computer vision 분야에서도 연구가 진행되어 왔다.
...
주석자들은 우리의 모델과 픽셀 단위의 정밀한 "브러시" 및 "지우개" 편집 도구를 사용하여 이를 수행했다.

실험을 통해 다양한 데이터셋에 비해 마스크 품질이 높다는 것을 인간 평가로 확인했고, automask로 모델을 훈련하는 것이 데이터 엔진이 생성한 모든 마스크를 사용하는 것에 거의 근접하다는 것을 알 수 있다.
### **Mask properties**
![[SAM_4.png]]
SA-1B의 객체 중심의 공간 분포를 기존의 가장 큰 분할 데이터셋과 비교한다. 

일반적인 사진작가의 편향이 모든 데이터셋에 존재함을 볼 수 있다.

SA-1B는 두 번째로 큰 Open Images에 비해 이미지 수가 11배, 마스크 수가 400배 많다.
