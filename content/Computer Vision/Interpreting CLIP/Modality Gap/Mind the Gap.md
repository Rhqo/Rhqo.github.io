...
다음으로, 서로 다른 랜덤 초기화가 원뿔 효과에 미치는 영향을 연구한다. \
그림 2(c)에서는 모델을 25번 랜덤 초기화한 후, UMAP 시각화를 통해 동일한 실제 데이터(즉, MSCOCO Caption)에서 추출한 임베딩을 나타냈다. \
각 랜덤 초기화는 뚜렷이 다른 원뿔을 형성함을 발견했다. \
이 현상은 다양한 신경망 아키텍처와 입력 모달리티(ResNet, vision or text transformer), ImageNet으로 사전 훈련된 모델, PCA 시각화 또는 랜덤 노이즈 입력에서도 유지된다. \
멀티모달 모델은 두 개의 인코더로 구성되어 서로 다른 랜덤 초기화에서 서로 다른 원뿔을 생성하기 때문에, 이는 초기화 시 모달리티 격차가 존재하는 이유를 설명한다.

> [!Tips]
> 모달리티 격차를 데이터 모달리티의 차이에 귀속시키는 것이 합리적으로 보일 수 있지만, 그림 2(c)는 두 인코더가 정확히 동일한 데이터를 동일한 모달리티에서 처리하더라도 격차가 여전히 존재함을 보여준다. \
> 따라서 격차는 다른 모달리티 없이도 존재할 수 있으며, 우리는 모달리티 격차 현상이 이해하는 데 사소하지 않다는 점을 강조한다.
